...
Note: In Jan 2021, I moved to my new laptop at home, made ready for Python 3, 
"""
...
def getvesselname(vessel_number):
  ...
  df=pd.read_csv('codes_temp.dat',sep='\s+',header=None)
...
def getlastwifi(sn):
  ...
  if (sn[0]!='1'): # case of Lowell Instruments where serial numbers starting with "1" are aquatec (change made 3/13/19)
    ...
    filenames=glob.glob('li_*.csv') # change made after lei Zhao's method was implemented in Aug 2019
...
def getlastfished(vn,mindistfromharbor):
   ...
   #df=pd.read_csv('emolt_ap3_reports.dat',sep=',',header=None) # this file is created during the "getap3.py" run
   df=pd.read_csv('http://nefsc.noaa.gov/drifter/emolt_ap3_reports.dat',sep=',',header=None) # this file is created during the "getap3.py" run
...
def get_FVCOM_bottom_temp(lati,loni,dtime,layer): # gets modeled temp using GOM3 forecast
        ...
        if itime==0:
            print('time is before fvcom forecast')
            return np.nan
        else:
...
outfile=open('lastfix_temp.html','w')
...
df=pd.read_csv('http://emolt.org/emoltdata/emolt.dat',sep='\s+',header=None) 
